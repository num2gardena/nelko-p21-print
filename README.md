...
> Currently, the only working deployments/configurations are:
> - **Linux:** Works with a local bridge (the Python Bluetooth/WebSocket bridge or a Docker deployment on a host PC) for the Bluetooth Classic connection.
> - **Windows:** Works directly using **Web Serial** in the browser. When you connect/pair your Nelko P21 in Windows Bluetooth settings, it becomes selectable in Windows as a serial device.
> - **Android/Mobile:** Work in progress (WIP) and currently untested/unverified.
